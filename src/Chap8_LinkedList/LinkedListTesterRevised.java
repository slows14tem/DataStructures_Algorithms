...
public class LinkedListTesterRevised {
	...
	static class Data {
		...
		public static final Comparator<Data> NO_NAME_ORDER = new DataOrderComparator();
		
		private static class DataOrderComparator implements Comparator<Data> {
			public int compare(Data d1, Data d2) {
				if (d1.no > d2.no) return 1;
				else if (d1.no < d2.no) return -1;
				else {
					return d1.name.compareTo(d2.name);
				}				
			}
	...
	static Menu SelectMenu() {
		...
		do {
			for (Menu m : Menu.values()) {
				...
				if ((m.ordinal() % 3) == 2 && m.ordinal() != Menu.TERMINATE.ordinal())	//줄바꿈
					System.out.println();
			}
			System.out.print(" : ");
			key = stdIn.nextInt();
...
//				list.delete(data,data.NO_NAME_ORDER);
				list.delete(data, data.NO_ORDER, data.NAME_ORDER);

				break;

			case SEARCH_NO: // 회원 번호 검색4
